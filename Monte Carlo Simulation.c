#include <stdio.h>
#include <stdlib.h>
#define SUITS 4
#define PIPS 13
#define DECK_SIZE (SUITS * PIPS)
#define HAND_SIZE 7

typedef enum suits { s, c, h, d } suit;

typedef struct a_card {
    suit suit;
    short pip;
} card;

struct hand_stats {
    unsigned long total;
    unsigned long no_pair;
    unsigned long one_pair;
    unsigned long two_pair;
    unsigned long three_of_kind;
    unsigned long full_house;
    unsigned long four_of_kind;
};

void shuffle_deck(card* deck, int len) {
    int i;
    for (i = 0; i < len; i++) {
        int j = rand() % (i + 1);
        card aux = deck[i];
        deck[i] = deck[j];
        deck[j] = aux;
    }
}

void evaluate_hand(card* hand, int len, struct hand_stats* stats) {
    unsigned value_count[PIPS] = { 0 };
    for (int i = 0; i < len; ++i) {
        ++value_count[hand[i].pip - 1];
    }

    unsigned count[SUITS + 1] = { 0 };
    for (int i = 0; i < PIPS; ++i) {
        ++count[value_count[i]];
    }

    if (count[4]) {
        ++stats->four_of_kind;
    }
    else if (count[3] && count[2] || count[3] >= 2) {
        ++stats->full_house;
    }
    else if (count[3]) {
        ++stats->three_of_kind;
    }
    else if (count[2] >= 2) {
        ++stats->two_pair;
    }
    else if (count[2]) {
        ++stats->one_pair;
    }
    else {
        ++stats->no_pair;
    }
    ++stats->total;
}

int main()
{
    card deck[DECK_SIZE];
    int i = 0;
    for (short j = 1; j <= PIPS; ++j) {
        for (short k = 1; k <= SUITS; ++k) {
            deck[i].suit = k;
            deck[i].pip = j;
            ++i;
        }
    }

    struct hand_stats stats = { 0, 0, 0, 0, 0, 0, 0 };
    const int DECKS_NUMBER = 1000000;
    for (int i = 0; i < DECKS_NUMBER; ++i) {
        shuffle_deck(deck, DECK_SIZE);
        for (int j = 0; j + HAND_SIZE < DECK_SIZE; j += HAND_SIZE) {
            evaluate_hand(deck + j, HAND_SIZE, &stats);
        }
    }

    /* now print the results */
    const long double total = stats.total;
    printf("No pair probability = %Lf\n", stats.no_pair / total);
    printf("One pair probability = %Lf\n", stats.one_pair / total);
    printf("Two pair probability = %Lf\n", stats.two_pair / total);
    printf("Three of a kind probability = %Lf\n", stats.three_of_kind / total);
    printf("Four of a kind probability = %Lf\n", stats.four_of_kind / total);
    printf("Full house probability = %Lf\n", stats.full_house / total);

    return 0;
}
